...
def define_dataset(logger, opt):
    ...
    if valid_split > 0.0 or 'debug' in opt['name']:
        ...
        if isinstance(valid_split, int):
            ...
            valid_len = int(data_len2 * valid_split)
        data_len2 -= valid_len
        drop_dataset, val_dataset = subset_split(dataset=val_dataset, lengths=[data_len2, valid_len], generator=Generator().manual_seed(opt['seed']))

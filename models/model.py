...
from my_util.psnr import calculate_psnr
class EMA():
...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ...
        # self.num_steps = len(self.phase_loader) * self.epoch
        # self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optG, T_max=self.num_steps)
        # self.warmup_scheduler = warmup.UntunedLinearWarmup(self.optG)
            ...
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    ...
                    if self.task in ['inpainting', 'uncropping']:
                        ...
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,phase='val')
                ...
                psnr_demo = calculate_psnr(self.gt_image, self.output)
                PSNR.append(psnr_demo)
                stacked_tensors = torch.stack(PSNR)
                PSNR_mean = torch.mean(stacked_tensors)
        return self.val_metrics.result(),PSNR_mean
            ...
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image,
                                                                                 y_0=self.gt_image, mask=self.mask,
                                                                                 sample_num=self.sample_num,phase='test')
                    else:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image,
                                                                                 sample_num=self.sample_num,phase='test')
                else:
                    if self.task in ['inpainting', 'uncropping']:
                        ...
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,phase='test')
